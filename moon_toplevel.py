...
class MoonWindow(ctk.CTkToplevel):
    systems = {}
    ...
    def __init__(self, master, planet_obj: Planet, system):
        ...
        if self.system in MoonWindow.systems.keys():
            MoonWindow.systems[self.system].append(self)
        else:
            MoonWindow.systems[self.system] = [self]
...
if __name__ == '__main__':
    ...
    for planet in planets:
        button = ctk.CTkButton(container,
                               ...
                               command=partial(lambda p: MoonWindow(container, p, "33 sextantis"), planet))
        button.grid()
        # planet_window =
    root.mainloop()
